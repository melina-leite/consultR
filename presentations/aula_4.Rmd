...
- A riqueza estimada pelo chao 1 é igual à riqueza observada, somada ao quadrado do número de espécies representadas por apenas um indivíduo (_singletons_), dividido pelo dobro do número de espécies com apenas dois indivíduos (_doubletons_). A mesma equação foi adaptada para utilizar o número de espécies que ocorrem respectivamente em uma ou em duas unidades amostrais (_uniques_ e _duplicates_ - chao 2). 
...
- Mais simples, considerado robusto

- Probabilidade de 2 indivíduos sorteados de uma comunidade pertencerem à mesma espécie
...
$D_{max} = S$ valor máximo do índice se todas as sps tivessem mesmo N indivíduos.
...
- Esta é uma técnica de ordenação da diversidade. 
...
- Usando a proporção de espécies em cada local: dividindo as abundâncias das espécies pela abundância total do local
...
- Distância zero: os dois locais são completamente similar. Distância 1: totalmente dissimilares (sem espécie em comum)
...
3. O "programa" então arranja os objetos ao longo de um ou mais eixos que melhor refletem os padrões encontrados nos dados. Estes eixos refletirão as variáveis ecológicas (ou espécies) que causaram os padrões nos dados. 
...
2. Ordenar dos locais nos eixos
...
- MDS1 foi capaz de representar bem o padrão da precipitação usando somente as distâncias entre os locais

- O eixo hipotético conseguiu predizer cerca de 67% (R²) da variação da precipitação.
...
## PCA em matriz de espécies {.columns-2}
```{r}

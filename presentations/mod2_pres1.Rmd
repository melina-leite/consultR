...
subtitle    : Módulo 2 Análises exploratórias de dados
author      : Melina Leite
...
## Inserindo/transformando dados
Inserindo abundância total em sps
```{r}
sps$abund <- rowSums(ilhas)
```
Transformando e inserindo log da abundância
```{r}
sps$log.abund <- log(sps$abund)
head(sps,2L)
```

## Centralizando e escalonando dados

(valor - média)/desvio padrão
```{r}
scale(sps$peso)
(sps$peso-mean(sps$peso))/sd(sps$peso)

sps$peso.scale <- scale(sps$peso)
```
Essa função server também para matrizes!
...
sps[ order(sps$peso) , ]
```
<!-- colocar mais exemplos de ordenação, sort, rank? -->
...
```{r}
aggregate(formula = abund ~ cor + dieta, data = sps, FUN = median)
```

## aggregate em data frames
abundância e peso médio por cor e dieta
```{r}
aggregate(cbind(abund, peso) ~ cor+ dieta, sps, median)
```


## Apply family
<!-- funções xapply e agregate estão na apostila bie5782 capt 4:http://ecologia.ib.usp.br/bie5782/doku.php?id=bie5782:03_apostila:04-dados   tb pag 56 e 61 Rbook-->
<!-- apostila vita capitulo 2 -->
...
# Índices de divesidade

## Shannon
O que é?
<!-- indice de shannon BIE5782 - tutorial 2  nesse tutorial tem tambem muito de estatística descritiva-->
Como calcular

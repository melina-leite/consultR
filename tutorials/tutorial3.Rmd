...
Vamos começar esboçando um problema inicial. Um pesquisador prentende medir a altura de árvores em uma floresta, pois ele tem algumas idéias sobre esta **distribuição de frequências de alturas** e gostaria de **inferir** sobre algumas características das árvores. É humanamente impossível medir todas as árvores da floresta (**população**), então ele pega uma **amostra aleatória** de árvores para prover as informações. Vamos ajudar esse pesquisador a analisar seus dados.
...
Nós modelamos estes dados como uma variável aleatória de distribuição normal. Porém, nós coletamos os dados de duas espécies diferentes, será que a altura de uma espécie nessa floresta é diferente da altura de outra espécie. Ou seja, será que as espécies pertencem a uma mesma distribuição e as diferenças encontradas são devido ao acaso ou será que cada espécie é uma variável aleatória com médias diferentes?
Nós podemos desenhar as distribuições de frequências das alturas separadamente para cada espécie:
...
Vemos então que podemos rejeitar H0, com uma probabilidade muito baixa de estarmos incorrendo no erro de tipo 1, e desta forma acreditamos que as espécies de árvores tenham distribuições de alturas diferentes.

 
O teste T possui três variantes básicas: a mais usada é normalmente chamada de teste t para amostras independentes, e permite a comparação de duas médias (a que acabamos de fazer); o teste t para uma amostra compara uma média com um valor fixo; e o teste t pareado (ou teste t para amostras dependentes) compara médias de unidades amostrais dependentes aos pares. Todos estas variações do teste t podem ser executadas no R pela função `t.test`, e o uso do teste t pareado e do teste t para uma amostra pode ser ativado com o uso dos argumentos `paired` e `mu`, respectivamente. Para saber mais como realizar estes testes no R, veja o capítulo 3 da apostila [Introdução ao uso do software R para as Ciências Biológicas](https://cantinhodor.files.wordpress.com/2015/03/introduc3a7c3a3o-ao-software-r-para-biologia-marcos-vital-ufal-marc3a7o-2015.pdf).
...
Mesmo fazendo parte de um mesmo conjunto de modelos, nas sessões seguintes vamos separar os modelos lineares pelos nomes tradicionais das análises: ANOVA e Regressão. Depois disso veremos alguns exemplos de GLMs aplicáveis a dados ecológicos.


## ANOVA

Vamos então incluir no nosso exemplo inicial uma terceira espécie amostrada (e vamos diminuir o número de amostras por espécie para ficar mais fácil de calcular):
```{r dados3}
...
A ANOVA irá então testar a hipótese nula de que as médias das 3 espécies não diferem. 
...
Agora nós sabemos de ondem vem estes números nesta tabela e como foi calculado o P, certo?

### Testes à posteriori

Perceba que a nossa hipótese nula na ANOVA que fizemos é de que as 3 médias não diferem. Ao rejeitá-la, então, nós apenas sabemos que pelo menos duas das médias são diferentes entre si, mas não sabemos quais são.
Alguns estatísticos dizem que uma análise gráfica é  suficiente para saber quais médias diferem ou não entre si (por exemplo os autores do livro Estatística sem Matemática). Entretanto, há também a possibilidade de se fazer testes à posteriori que vão comparar as médias das espécies par a par. Vejamos aqui o mais utilizado **teste de Tukey HDS**, chamado pela função `TukeyHSD`, que atua diretamente no objeto com o resultado da ANOVA:
...
stripchart( arv3$alt~arv3$sp,
           vertical=TRUE, pch=16, col=c("black", "green", "red"))

#boxplot
boxplot(arv4)
```

Nossa análise então indicou que há diferenças entre as médias das alturas das árvore e que a espécie 1 não difere da espécie 2, mas difere da espécie 3, enquanto a espécie 2 difere da espécie 3. 
...
Os resultados são parecidos com os de uma ANOVA, mas conta com alguns detalhes a mais. No exemplo acima, a linha (Intercept) refere-se ao intercepto da equação linear (o b, na equação y = ax + b), e verifica a hipótese nula de que ele seria igual a zero; de uma forma geral, nós raramente nos preocupamos com a esta parte do resultado, pois o intercepto normalmente não faz parte da nossa pergunta. Já a linha abaixo, com o nome da variável explicativa (nutri), é sempre do nosso interesse: ela verifica a hipótese nula de que o parâmetro a da equação seja igual a zero; ou seja, ao rejeitarmos a hipótese nula nós concluímos que de fato há uma relação estatisticamente significativa entre as duas variáveis, e que o efeito desta relação é medido por este parâmetro. Por fim, a interpretação da correlação, que está representada no parâmetro Adjusted R- squared também é importante, e mede o quanto da variação na variável resposta foi de fato explicada pela variável explicativa. Confuso? Então vejamos os resultados do exemplo acima: o teste rejeita a hipótese nula de que a altura das árvores não varia em função da quantidade de nutrientes (o valor de p menor do que 0.05). O resultado da regressão permite concluirmos que, em média, cada incremento de uma unidade de área aumenta a riqueza em 0,34 (este é o valor de a da equação). Por fim, o valor de R2 ajustado nos diz que 24.15% da altura da árvore é explicada pela quantidade de nutrientes do local.
...
Com os dados das árvores nós testamos independentemente duas perguntas: se a altura média das árvores era diferente para cada espécie e se a altura das árvores dependia da quantidade de nutrientes no solo. Mas porque fazer estas análises separadamente? Ambas as variáveis independentes podem influenciar na altura das árvores e, para isso, precisamos modelá-las em conjunto.

O modelo linear proposto abaixo é muitas vezes chamado de ANCOVA, ou seja, Análise de Covariância, que tem então duas variáveis explicativas, uma categórica (no nosso caso a espécie de planta) e uma contínua (os nutrientes).
...
Existem duas notações de fórmula para este modelo, que devem dar o mesmo resultado. 
```{r}
...
Olhando a tabela de ANOVA deste modelo vemos que a interação não foi significativa, e que apenas os nutrientes são responsáves pela variação no na altura das árvores:
```{r}
...
Os modelos estatísticos que lidamos até agora tem como pressuposto (premissa) a distribuição Normal da variável resposta e homogeneidade de variâncias (tb conhecido como homocedasticidade). No entanto, em muitas situações a suposição de normalidade e varância constante não é plausível. Conseqüentemente, o uso de métodos que assumem a normalidade pode ser insatisfatório e aumentam a probabilidade de cometermos erros inferenciais (erros do Tipo I e II). Nestes casos, outras alternativas que não pressupoem distribuição normal dos dados são atraentes e mais robustas.
É aí que entram os GLMs para modelar os dados quando a variância não é constante, e/ou quando os erros não são normalmente distribuídos. Muitos tipos de dados têm erros não normais. No passado, as únicas maneiras capazes de lidar com esse problema eram a transformação da variável resposta ou a adoção de métodos não paramétricos. Em GLM, assumimos que cada resultado da variável dependente Y seja gerado a partir de uma variedade de diferentes tipos de distribuições que lidam com esse problema:  
...
3. A relação entre o valor médio de Yi e a parte sistemática. Esta é também chamada de ligação entre a média e a parte sistemática (Tabelas 2 e 3).
  

VER CAPITULO 13 DO CRAWLEY PRA VE SE ELE EXPLICA MELHOR




# Checando os modelos
Crawley pag 404 e 418
testes de premissas,  apostil vital pag 43 (levene, shapiro wilk)

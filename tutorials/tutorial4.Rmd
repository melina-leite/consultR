---
title: 'Roteiro 4 - Analisando Comunidades'
author: "Melina Leite"
...
Para construir a curva de acumulação de espécies vamos usar o pacote `BiodiversityR`, que além de poder ser usado diretamente no R, tem também uma interface amigável através do pacote `Rcmdr`. Veja o manual do pacote [aqui](http://www.worldagroforestry.org/downloads/Publications/PDFS/b13695.pdf). 
...
[Gotelli & Colwell (2001)](http://onlinelibrary.wiley.com/doi/10.1046/j.1461-0248.2001.00230.x/epdf) descrevem dois tipos de curva, a **curva do coletor** e a **curva de rarefação**. A curva acima é curva do coletor, onde a riqueza de espécies foi plotada de acordo com a ordem dos locais amostrados (argumento `method="collector"). 
...
O valor do perfil quando **alfa = infinito** provém informação da proporção da espécie mais abundante. Por exemplo, para local C, o valor quando alfa=infinito é 0.55, quando tiramos o anti-logarítmo do valor recíproco, obtemos a proporção da espécies dominante, que neste caso é (sp1) 4/7 =  0.57. Vemos que 4/7 = 1/exp(0.55). Perfis que tem maior alpha=infinito, tem menor proporção da espécie dominante, ou seja são mais equitativas.
...
A distância euclidiana não é uma boa distância ecológica para dados brutos da matriz de espécie - com as abundâncias de cada espécie em cada local. Portanto, o que se recomenda é transformar a matriz de espécies, por exemplo padronizando os dados antes de calcular as distâncias (veja abaixo roteiro PCA com um exemplo de transformação). 
...
Vamos descrever a seguir duas das mais utilizadas análises de ordenação. Exemplificaremos o Escalonamento Multidimensional Não Paramétrico (NMDS) e a Análise de Componentes Principais, que são métodos não-restritivos (_'unconstrained'_ em inglês) que usa as distâncias ecológicas baseada apenas na matriz de espécies (ou apenas na matriz de variáveis ambientais para PCA com dados ambientais). 
...
**OBS**: uma outra forma de fazer os plots acima em um só comando é usando a função `cleanplot.pca` disponível nesse [arquivo](https://www.dropbox.com/s/v15zq2mtsjjs6b1/cleanplot.pca.R?dl=0) (também retirada do livro "Numerical Ecology with R". Baixe o arquivo e coloque ele na pasta da sua área de trabalho:
```{r, echo=F}
...
- [Borcard et al. 2011. Numerical Ecology with R.](https://www.dropbox.com/s/btwmawmyewyzu80/Numerical%20ecology%20with%20R.pdf?dl=0)

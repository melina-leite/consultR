...
Uma forma de caracterizar uma comunidade é simplesmente contar ou listar as espécies presentes. Parece um procedimento fácil e objetivo descrever e comparar comunidades por meio de suas 'riquezas' em espécies. Na prática, entretanto, isso é surpreendentemente difícil, porque em geral apenas um subamostra dos organismos em uma área pode ser contada. O número de espécies depende do número de amostrar obtidas, então quando devemos cessar a amostragem? Idealmente até que o número de espécies alcance um platô. A riqueza de espécies em diferentes comunidades deve ser comparada usando-se um mesmo tamanho de amostragem.
...
[Gotelli & Colwell (2001)](http://onlinelibrary.wiley.com/doi/10.1046/j.1461-0248.2001.00230.x/epdf) descrevem dois tipos de curva, a **curva de acumulação** e a **curva de rarefação**. A curva acima é curva de acumulação, onde a riqueza de espécies foi plotada de acordo com a ordem dos locais amostrados. 
...
Para alguns conjuntos de dados, a informação não é coletada por local, e sim por indivíduo observado. Frequentemente, levantamentos de vertebrados são obtidos dessa maneira, registrando as informações das espécies para cada indivíduo observado sequencialmente. Entretanto, se dados baseados em amostra estão disponíveis, podmeos fazer curvas de rarefação baseadas tanto em **indivíduos** quanto baseadas em **amostras**.
...
iv) é um método para estimar a riqueza de espécies em uma amostra menor – não pode ser usado para extrapolar e estimar riqueza.
...
mat <- matrix(c(1,5,0,1,5,0,0,0,1), nrow=3, dimnames = list(c("A","B","C"), c("sp1","sp2","sp3")))

#vendo a matriz de distância entre os locais
...
Os métodos de ordenação arranjam as comunidades (locais) de forma que as distâncias entre elas no gráfico representem suas distâncias ecológicas. O resultado da ordenação é geralmente visualizado como um gráfico de duas dimensões. Nestes gráficos, cada local é apresentado e os locais próximos são interpetados como tendo composição de espécies similar, enquanto que locais afastados contém composição de espécies diferente.

Vamos descrever a seguir duas das mais utilizadas análises de ordenação. Exemplificaremos o Escalonamento Multidimensional Não Paramétrico (NMDS) e a Análise de Componentes Principais, que são métodos não-restritivos (_'unconstrained'_ em inglês) que usa as distâncias ecológicas baseada apenas na matriz de espécies. 


## Análise de componentes principais (PCA)

A análise de componentes principais (PCA) é principalmente usada para reduzir a dimensionalidade dos dados, e também verificar como as amostras se relacionam, ou seja, quão semelhante são segundo as variáveis utilizadas. O resultado prático é produzir um diagrama de ordenação que sintetize os dados, no qual os objetos mais próximos são mais semelhantes. Diferentemente de outras análises de ordenação, só é possível usar a distância euclidiana como coeficiente de similaridade na PCA. Logo, é mais recomendado usá-la para analisar variáveis ambientais ou medidas morfológicas.


A APOSTILA UNESP FAZ PCA PARA DADOS DAS VARIAVEIS AMBIENTAIS

O MANULA DO BIOD. FAZ PCA PARA DADOS DAS ESPECIES...



Voltamos com os dados da vegetação de dunas e vamos calcular o PCA:
```{r}
pca1 <- rda(dune)
summary(pca1)

plot1 <- ordiplot(pca1, scaling=1, type="text")
plot2 <- ordiplot(pca1, scaling=2, type="text")
```
...
inertcomp(pca1, display='species', statistic="explained", proportional = F)
```

```{r}
#Calculating the proportion of variance explained for an ordination graph
goodness(pca1, display='site', choices = c(1:2), statistic = 'explained')
```
```{r}
#Adding a vector and perpendicular lines for a particular species to an ordination plot
ordivector(plot1, "Agrostol", lty=2)
```

```{r}
# Calculating correlation among vectors
cor.test(dune[,"Alopgeni"], dune[,"Agrostol"])
```

```{r}
#Calculating the number of ecological meaningful principal components
PCAsignificance(pca1, axes=30)
```
```{r}
#drawing an equilibrium circle
ordiequilibriumcircle(pca1,plot1)
```
...
model1 <- add.spec.scores(model1, dune, method='wa.scores')
model1
plot5 <- ordiplot(model1)
```

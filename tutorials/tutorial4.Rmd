...
Vejamos as propriedades de alguns estimadores (para mais detalhes ver [Santos 2003](https://www.dropbox.com/s/8mzru9p4whvoo00/Santos_2003_Estimativas%20de%20riqueza%20em%20especies.pdf?dl=0):
...
- **Chao 1 e 2**: a riqueza estimada pelo chao 1 é igual à riqueza observada, somada ao quadrado do número de espécies representadas por apenas um indivíduo (singletons), dividido pelo dobro do número de espécies com apenas dois indivíduos (doubletons). A mesma equação foi adaptada para utilizar o número de espécies que ocorrem respectivamente em uma ou em duas unidades amostrais (uniques e duplicates - chao 2).
...
Vamos descrever a seguir duas das mais utilizadas análises de ordenação. Exemplificaremos o Escalonamento Multidimensional Não Paramétrico (NMDS) e a Análise de Componentes Principais, que são métodos não-restringidos (_'unconstrained'_ em inglês) que usa as distâncias ecológicas baseada apenas na matriz de espécies. Também veremos , um método restringido (_'constrained"_") que usa as variáveis ambientais para guiar a ordenação.  


## Análise de componentes principais (PCA)

A análise de componentes principais utiliza as distâncias euclidianas para investigar a distância ecológica entre os locais.  
